...
template<typename T>
SRADBReader<T>::SRADBReader(const char *dataFileName_, const char *indexFileName_, int threads, int dataMode) :
...
template<typename T>
SRADBReader<T>::SRADBReader(SRADBReader<T>::Index *index, size_t size, size_t dataSize, T lastKey,
                            int dbType, unsigned int maxSeqLen, int threads) :
        threads(threads), dataMode(USE_INDEX), dataFileName(NULL), indexFileName(NULL),
        size(size), dataFiles(NULL), dataSizeOffset(NULL), dataFileCnt(0), totalDataSize(0), dataSize(dataSize),
        lastKey(lastKey),
        maxSeqLen(maxSeqLen), closed(1), dbtype(dbType), compressedBuffers(NULL), compressedBufferSizes(NULL),
        index(index), sortedByOffset(true),
        id2local(NULL), local2id(NULL), dataMapped(false), accessType(NOSORT), externalData(true), didMlock(false) {}

template<typename T>
void SRADBReader<T>::setDataFile(const char *dataFileName_) {
...
template<typename T>
void SRADBReader<T>::readMmapedDataInMemory() {
...
template<typename T>
void SRADBReader<T>::printMagicNumber() {
    Debug(Debug::INFO) << magicBytes << "\n";
}

template<typename T>
SRADBReader<T>::~SRADBReader() {
    if (dataFileName != NULL) {
        free(dataFileName);
    }
...
template<typename T>
bool SRADBReader<T>::open(int accessType) {
    ...
    if (externalData == false) {
        ...
        Util::checkAllocation(index, "Cannot allocate index memory in SRADBReader");
...
template<typename T>
void SRADBReader<T>::sortIndex(bool) {
}

template<typename T>
bool SRADBReader<T>::isSortedByOffset() {
    return sortedByOffset;
}

template<>
void SRADBReader<std::string>::sortIndex(bool isSortedById) {
    if (accessType == SORT_BY_ID) {
        ...
        if (accessType != NOSORT && accessType != HARDNOSORT) {
            Debug(Debug::ERROR) << "SRADBReader<std::string> cannot be opened in sort mode\n";
            EXIT(EXIT_FAILURE);
        }
    }
}
...
template<>
void SRADBReader<unsigned int>::sortIndex(bool isSortedById) {
...
template<typename T>
char *SRADBReader<T>::mmapData(FILE *file, size_t *dataSize) {
    struct stat sb;
...
template<typename T>
size_t SRADBReader<T>::bsearch(const Index *index, size_t N, T value) {
    Index val;
    val.id = value;
...
template<typename T>
char *SRADBReader<T>::getDataCompressed(size_t id, int thrIdx) {
...
template<typename T>
size_t SRADBReader<T>::getAminoAcidDBSize() {
    checkClosed();
...
template<typename T>
char *SRADBReader<T>::getData(size_t id, int thrIdx) {
    if (compression == COMPRESSED) {
...
template<typename T>
char *SRADBReader<T>::getDataUncompressed(size_t id) {
    checkClosed();
    if (!(dataMode & USE_DATA)) {
        Debug(Debug::ERROR) << "SRADBReader is just open in INDEXONLY mode. Call of getData is not allowed" << "\n";
        EXIT(EXIT_FAILURE);
    }
    if (id >= size) {
...
template<typename T>
char *SRADBReader<T>::getDataByOffset(size_t offset) {
...
template<typename T>
void SRADBReader<T>::touchData(size_t id) {
    if ((dataMode & USE_DATA) && (dataMode & USE_FREAD) == 0) {
        char *data = getDataUncompressed(id);
...
template<typename T>
char *SRADBReader<T>::getDataByDBKey(T dbKey, int thrIdx) {
...
template<typename T>
size_t SRADBReader<T>::getLookupSize() const {
    checkClosed();
    return lookupSize;
}

template<typename T>
size_t SRADBReader<T>::getSize() const {
    checkClosed();
    return size;
}

template<typename T>
T SRADBReader<T>::getDbKey(size_t id) {
    checkClosed();
    if (id >= size) {
...
template<typename T>
size_t SRADBReader<T>::getLookupIdByKey(T dbKey) {
    if ((dataMode & USE_LOOKUP) == 0) {
        Debug(Debug::ERROR) << "SRADBReader for datafile=" << dataFileName << ".lookup was not opened with lookup "
                                                                              "mode\n";
        EXIT(EXIT_FAILURE);
    }
    LookupEntry val;
...
template<typename T>
size_t SRADBReader<T>::getLookupIdByAccession(const std::string &accession) {
...
template<typename T>
T SRADBReader<T>::getLookupKey(size_t id) {
    if (id >= lookupSize) {
        Debug(Debug::ERROR) << "Invalid database read for id=" << id << ", database index=" << dataFileName
                            << ".lookup\n";
...
template<typename T>
std::string SRADBReader<T>::getLookupEntryName(size_t id) {
    if (id >= lookupSize) {
        Debug(Debug::ERROR) << "Invalid database read for id=" << id << ", database index=" << dataFileName
                            << ".lookup\n";
...
template<typename T>
unsigned int SRADBReader<T>::getLookupFileNumber(size_t id) {
    if (id >= lookupSize) {
        Debug(Debug::ERROR) << "Invalid database read for id=" << id << ", database index=" << dataFileName
                            << ".lookup\n";
...
template<>
void SRADBReader<unsigned int>::lookupEntryToBuffer(std::string &buffer, const LookupEntry &entry) {
...
template<>
void SRADBReader<std::string>::lookupEntryToBuffer(std::string &buffer, const LookupEntry &entry) {
...
template<typename T>
size_t SRADBReader<T>::getId(T dbKey) {
...
template<typename T>
size_t SRADBReader<T>::maxCount(char c) {
    checkClosed();
...
template<typename T>
void SRADBReader<T>::checkClosed() const {
    if (closed == 1) {
...
template<typename T>
bool SRADBReader<T>::readIndex(char *data, size_t indexDataSize, Index *index, size_t &dataSize) {
#ifdef OPENMP
    int threadCnt = 1;
...
template<typename T>
T SRADBReader<T>::getLastKey() {
    return lastKey;
}

template<>
void SRADBReader<std::string>::readIndexId(std::string *id, char *line, const char **cols) {
    ptrdiff_t keySize = ((cols[1] - 1) - line);
    id->assign(line, keySize);
}

template<>
void SRADBReader<unsigned int>::readIndexId(unsigned int *id, char *, const char **cols) {
    *id = Util::fast_atoi<unsigned int>(cols[0]);
}

template<>
unsigned int SRADBReader<std::string>::indexIdToNum(std::string *id) {
    return id->size();
}

template<>
unsigned int SRADBReader<unsigned int>::indexIdToNum(unsigned int *id) {
    return *id;
}

template<typename T>
void SRADBReader<T>::unmapData() {
    if (dataMapped == true) {
        for (size_t fileIdx = 0; fileIdx < dataFileNames.size(); fileIdx++) {
...
template<typename T>
size_t SRADBReader<T>::getDataOffset(T i) {
    size_t id = bsearch(index, size, i);
    return index[id].offset;
}

template<>
size_t SRADBReader<unsigned int>::indexMemorySize(const SRADBReader<unsigned int> &idx) {
    size_t memSize = // size + dataSize
            2 * sizeof(size_t)
            // maxSeqLen + lastKey + dbtype
            + 3 * sizeof(unsigned int)
            // index
            + idx.size * sizeof(SRADBReader<unsigned int>::Index)
            // seqLens
...
template<>
char *SRADBReader<unsigned int>::serialize(const SRADBReader<unsigned int> &idx) {
    ...
    p += sizeof(unsigned int);
    memcpy(p, idx.index, idx.size * sizeof(SRADBReader<unsigned int>::Index));
    p += idx.size * sizeof(SRADBReader<unsigned int>::Index);
    return data;
}

template<>
SRADBReader<unsigned int> *SRADBReader<unsigned int>::unserialize(const char *data, int threads) {
    const char *p = data;
    ...
    p += sizeof(unsigned int);
    SRADBReader<unsigned int>::Index *idx = (SRADBReader<unsigned int>::Index *) p;
    p += size * sizeof(SRADBReader<unsigned int>::Index);

    return new SRADBReader<unsigned int>(idx, size, dataSize, lastKey, dbType, maxSeqLen, threads);
}

template<typename T>
void SRADBReader<T>::setData(char *data, size_t dataSize) {
    if (dataFiles == NULL) {
...
template<typename T>
void SRADBReader<T>::setMode(const int mode) {
    this->dataMode = mode;
}

template<typename T>
size_t SRADBReader<T>::getOffset(size_t id) {
    if (id >= size) {
        Debug(Debug::ERROR) << "Invalid database read for id=" << id << ", database index=" << indexFileName << "\n";
...
template<typename T>
size_t SRADBReader<T>::findNextOffsetid(size_t id) {
...
template<typename T>
int SRADBReader<T>::isCompressed(int dbtype) {
    return (dbtype & (1 << 31)) ? COMPRESSED : UNCOMPRESSED;
}

template<typename T>
void SRADBReader<T>::setSequentialAdvice() {
...
template<typename T>
void SRADBReader<T>::readLookup(char *data, size_t dataSize, SRADBReader::LookupEntry *lookup) {
    size_t i = 0;
    size_t currPos = 0;
...
template<typename T>
void SRADBReader<T>::moveDatafiles(const std::vector<std::string> &files, const std::string &destination) {
...
template<typename T>
void SRADBReader<T>::moveDb(const std::string &srcDbName, const std::string &dstDbName) {
...
template<typename T>
void SRADBReader<T>::removeDb(const std::string &databaseName) {
...
template<typename T>
void SRADBReader<T>::softlinkDb(const std::string &databaseName, const std::string &outDb, DBFiles::Files
dbFilesFlags) {
    copyLinkDb(databaseName, outDb, dbFilesFlags, true);
}

template<typename T>
void SRADBReader<T>::copyDb(const std::string &databaseName, const std::string &outDb, DBFiles::Files dbFilesFlags) {
    copyLinkDb(databaseName, outDb, dbFilesFlags, false);
}

template<typename T>
void SRADBReader<T>::decomposeDomainByAminoAcid(size_t worldRank, size_t worldSize, size_t *startEntry, size_t
*numEntries) {
...
template<typename T>
void SRADBReader<T>::copyLinkDb(const std::string &databaseName, const std::string &outDb, DBFiles::Files dbFilesFlags,
                                bool link) {
...
template
class SRADBReader<unsigned int>;

template
class SRADBReader<std::string>;
