...
public:
    ...
    SRADBReader(Index* index, size_t size, size_t aaDbSize, T lastKey,
    ...
    static size_t indexMemorySize(const SRADBReader<unsigned int> &idx);

    static char* serialize(const SRADBReader<unsigned int> &idx);

    static SRADBReader<unsigned int> *unserialize(const char* data, int threads);
...
private:
    ...
    static void copyLinkDb(const std::string &databaseName, const std::string &outDb, DBFiles::Files dbFilesFlags, bool
    link);
