...
int computeAlignments(int argc, const char **argv, const Command& command){
    ...
    DBReader<unsigned int> resultReader(par.db3.c_str(), par.db3Index.c_str(), par.threads, DBReader<unsigned int>::USE_DATA 
        | DBReader<unsigned int>::USE_INDEX | DBReader<unsigned int>::USE_WRITABLE);
